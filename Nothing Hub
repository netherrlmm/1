...
local Window = create:Win("Nothing Hub")

game:GetService("CoreGui").redui.MainSceen.Visible = false

create:Notifile("Nothing Hub", "Welcome to Nothing Hub", 5)
...
function loadcheck()
    if isfile("Nothing Hub.json") then
        g = readfile("Nothing Hub.json")
            return g
        else
            writefile("Nothing Hub.json",game:GetService("HttpService"):JSONEncode(getgenv().Save))
        return
    end
end
getgenv().Save = {
...
function LoadSetting()
    if isfile("Nothing Hub.json") then
            vb = game:GetService("HttpService"):JSONDecode(readfile("Nothing Hub.json"))
            getgenv().Save = vb
                return vb
        else
            loadcheck()
 ...
 page6:Drop("Choose NPC" , false, {
    ...
    then 
        game.Players.LocalPlayer.Character.HumanoidRootPart.CFrame = game:GetService("Workspace").Merchants.QuestHakiMerchant.HumanoidRootPart.CFrame
            ...
            create:Notifile("Nothing Hub", "Rayleigh did not spawn.!", 5)
            end
...
page8:Toggle("TP TO PLAYER",getgenv().Save["TPPLAYER"],function(bool)
    ...
    if (plr2.Character.HumanoidRootPart.Position - game:GetService("Workspace").CustomizeModel.SpawnAreaStuffK.HBase.Position).Magnitude < 1000 and getgenv().Save["Players"] ~= nil and getgenv().Save["Players"] ~= "" then
        create:Notifile("Nothing Hub", "Spawn Zone", 3)
    end
    if getgenv().Save["ChooseMet"] == "" then
        create:Notifile("Nothing Hub", "Choose Method TP first", 3)
    end
    if getgenv().Save["AutoSave"] then
        SaveSetting()
    end
...
page9:Toggle("Auto Mod" , getgenv().Save["Mod"], function(m)
    create:Notifile("Nothing Hub" , "You Will Got Protect Now", 5)
    getgenv().Save["Mod"] = m
...
page7:Button("Set Spawn", function()
    ...
    create:Notifile("Nothing Hub", "Pos Spawn: " ..tostring(getgenv().Save["posFarm"]), 3)
end)
...
page1:Button("Melee V1 (500 melee can use it)", function()
    ...
    create:Notifile("Nothing Hub", "500 Melee Can Use", 3)
    if getgenv().Save["AutoSave"] then
        SaveSetting()
    end
end)
...
page3:Button("Test WebHook",function()
    pcall(function()
        ...
        ["embeds"] = {
            {
                ["title"] = " Nothing Hub Test WebHook",
...
page1:Toggle("FastAttack",getgenv().Save["FastAttack"],function(x)
    ...
    if ( not plr.Character:FindFirstChild("Yoru")) then
        create:Notifile("Notification", "U need Equip Yoru", 5)
...
page1:Toggle("Auto FixSpam",getgenv().Save["FixSpam"],function(x)
    ...
    create:Notifile("Nothing Hub", "Set Skill Fruit U want Spam (C button)", 5)
end)
...
page7:Drop("Select Method Farm" ,false, {"Sword or no burning damage like magma, gas...", "Burning damage like magma, gas..."}, function(t)
    ...
    if getgenv().methodbty == "Sword or no burning damage like magma, gas..." then
        create:Notifile("Nothing Hub", "Sword or no burning damage like magma, gas...", 3)
    elseif getgenv().methodbty == "Burning damage like magma, gas..." then
        create:Notifile("Nothing Hub", "Burning damage like magma, gas...", 3)
    end
    if getgenv().Save["AutoSave"] then
        SaveSetting()
    end
...
for _, v in pairs(Players:GetPlayers()) do 
    if table.find(BlacklistedPlayers, v.Name) then 
        Players.LocalPlayer:Kick("Staff OPL Has Join Server , Dont Worry. You Got Protect By NeonHub")
    end
end
...
messagebox("Welcome To Nothing Hub","Nothing Hub",0)
